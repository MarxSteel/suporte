...
$FU->execute();
?>
<table id="APG" class="table table-hover table-striped table-responsive" cellspacing="0" width="100%">
 <thead>
  <tr>
   <td width="5%">Cham.</td>
  ...
  <?php while ($FUser = $FU->fetch(PDO::FETCH_ASSOC)): 
   ...
   echo '<td>';
    ...
    echo '<a class="btn btn-warning btn-xs" href="';
     echo "javascript:abrir('AtGer.php?ID=" . $FUser["id"] . "');";
    ...
    echo '<a class="btn btn-default btn-xs" href="';
     ...
     {
      echo '<button type="button" class="btn btn-success btn-xs" data-toggle="modal" data-target="#exampleModal" data-whatever="' . $FUser["id"] . '" data-idvalue="' . $FUser["id"] . '" data-botao="FINALIZAR" data-obs="' . $FUser["DescAtend"] . '"><i class="fa fa-check"></i></button>';
     }
     else{
