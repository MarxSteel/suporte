     ...
     <div class="box-body">
      <div class="col-xs-12">
       <form name="IItem" id="name" method="post" action="" enctype="multipart/form-data">
         <input name="IItem" type="submit" class="btn btn-danger btn-flat btn-block btn-lg" id="IItem" value="INATIVAR DOWNLOAD DO ARQUIVO"  /> 
       </form>
       <?php
        if(@$_POST["IItem"])
        {     
        $DataAtual = date('Y/m/d - H:i:s'); //TRATANDO DATA E HORA, DD/MM/YYYY - HH:MM:SS
         $Recebe = $PDO->query("UPDATE firmware SET Status='0' WHERE id='$id'");
         if ($Recebe)
          {
          $TpLog = "Inativado Firmware. Cod: " . $id;
     	  $InsLog = $PDO->query("INSERT INTO log (Cod, TipoLog, DataCadastro, UserCadastro) VALUES ('3', '$TpLog', '$DataAtual', '$NomeUserLogado')");
        	if ($InsLog) 
        	{
